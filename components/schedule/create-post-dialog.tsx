...
const CreatePostDialog = ({
 ...
 useEffect(() => {
  if (
   channels.length > 0 &&
   Object.keys(channelContent).length === 0
  ) {
 ...
 const previewChannel =
  channels.find((c) => c.id === activePreview) ?? null;
     ...
     <DialogHeader className="px-8 py-3 border-b">
      <div className="flex items-center justify-between">
       ...
       <div className="flex items-center gap-px">
        {rightTabs.map((tab) => (
         <Tooltip key={`right-tab-${tab.id}`}>
          <TooltipTrigger asChild>
           <Button
            variant={
             selectedRightTab === tab.id ? 'default' : 'ghost'
            }
        ...
        <div className="flex flex-wrap gap-4">
         {isPending
          ...
          : channels?.map((channel) => {
             ...
             return (
              <Tooltip key={`channel-${channel.id}`}>
               <TooltipTrigger asChild>
                <button
                 key={channel.id}
                 style={
